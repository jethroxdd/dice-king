...
func _ready():
	...
	# Атакующая кость (D4) с рунами урона на всех гранях
	var poison_die = Die.new(
		4, 
		[ 
			PoisonRune.new(), 
			PoisonRune.new(), 
...
func create_buttons(button_count: int):
	...
	for die in player.dice:
		dice_tooltip_text.append(die.tooltip_text)
